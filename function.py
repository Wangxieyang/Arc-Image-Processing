...
def connect(func, dir):
    imgList = os.listdir('./test1/' + dir)
    # print(imgList)

    sum = 0
    for i in range(0, len(imgList)):
        image = cv2.imread("./test1/" + dir + "/" + imgList[i])
        temp = func(image)
...
def width_draw(img):
    ...
    for i in range(3):
        mid = ss.medfilt2d(gauss, [3, 3])
    # cv2.imshow('mid', mid)
    _, binary = cv2.threshold(mid, 200, 255, cv2.THRESH_BINARY)
    # cv2.imshow("binary", binary)
    kernal = np.ones((3, 3), np.uint8)

    erosion = cv2.erode(binary, kernal)
    # cv2.imshow('erosion', erosion)
    count1 = count(erosion)
    # ret0, dio = cv2.threshold(erosion, 0, 255, cv2.THRESH_OTSU)  ##cv2.THRESH_BINARY +
    # cv2.imshow('dio',dio)
    ...
    s=count1[0]/784
    s=round(s,2)
    cv2.line(img, (250, itmp), (500, itmp), (0, 0, 255), 1)
    cv2.putText(img, "w=" + str(dis) + "mm", (225, itmp - 10), 0, 0.6, (255, 255, 255), 1, 1)
    cv2.putText(img, "s=" + str(s) + "mm2", (225, itmp + 15), 0, 0.6, (255, 255, 255), 1, 1)
    cv2.imshow("img", img)
